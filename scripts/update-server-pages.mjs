...
function statusBlock(status) {
  if (!status) {
    return '服务器公开状态暂时无法获取。'
  }

  if (!status.online) {
    return '服务器公开状态：离线或暂时无法从状态接口访问。'
  }
...
${data.name} ${data.season} 是服务器当前运行中的周目。你可以通过 \`${data.address}:${data.bedrockPort}\` 在 Minecraft 基岩版加入；Java 版玩家也可以通过互通入口进入服务器。
...
OWL Server 当前运行周目为 **${data.season}**。你可以在下方找到服务器地址，并将服务器一键导入游戏。
...
${data.name} ${data.season} 是当前运行中的${data.type}，面向 Java 与基岩玩家开放。X4 的定位不是大型快餐服，而是一个正在发展中的长期社区：玩家可以一起建设主城、发展领地、参与轻经济系统，并在一个相对稳定的环境里慢慢经营自己的生活区。
